module.exports = (function(){
   ...
   function DBManagerBuilder(fileName){
      ...
      var path = (fileName.indexOf("/")==0) ? fileName : "/sdcard/kbot/"+fileName;
      const myDB = android.database.sqlite.SQLiteDatabase.openDatabase(path, null, android.database.sqlite.SQLiteDatabase.CREATE_IF_NECESSARY);
      ...
      function makeBackup() {
         const date=new Date();
         var is=new java.io.FileInputStream("/sdcard/kbot/"+fileName);
         var os=new java.io.FileOutputStream("/sdcard/kbot/"+fileName+date.getFullYear()+"_"+(date.getMonth()+1)+"_"+date.getDate()+"_"+date.getHours()+"_"+date.getMinutes()+"_"+date.getSeconds());
         var len=0;
      ...
      function makeArgs(args){
         return (args!=undefined || args!=null) ? ((args instanceof Array) ? args : [args]) : null;
      }
      ...
      function rawQueryForObject(sql,selectionArgs,stringFlag){
         try{
            return cursorToObject(rawQueryForCursor(sql,selectionArgs),stringFlag);
         }catch(e){throw e;}
      }
      rawQueryForObject.toString=()=>'DBManager.rawQueryForObject(string sql)\nDBManager.rawQueryForObject(string sql, string[] selectionArgs, boolean stringFlag)\n'
         +'sql을 실행합니다. select의 수행에 사용하며 insert, update, delete, create, alter등은 execSQL을 이용합니다.\n'
         +'sql 문자열에 ? 가 포함되어 있으면 해당 부분은 selectionArgs의 값으로 대체됩니다.\n'
         +'stringFlag의 값이 true일 경우 column의 type이 String이 아니더라도 String으로 반환됩니다.\n'
         +'수행에 성공할 경우 object로 이루어진 array를 리턴하며, 실패할 경우 에러 e를 throw합니다.';
      function rawQueryForArray(sql,selectionArgs,stringFlag){
         try{
            return cursorToArray(rawQueryForCursor(sql,selectionArgs),stringFlag);
         }catch(e){throw e;}
      }
      rawQueryForArray.toString=()=>'DBManager.rawQueryForArray(string sql)\nDBManager.rawQueryForArray(string sql, string[] selectionArgs, boolean stringFlag)\n'
         +'sql을 실행합니다. select의 수행에 사용하며 insert, update, delete, create, alter등은 execSQL을 이용합니다.\n'
         +'sql 문자열에 ? 가 포함되어 있으면 해당 부분은 selectionArgs의 값으로 대체됩니다.\n'
         +'stringFlag의 값이 true일 경우 column의 type이 String이 아니더라도 String으로 반환됩니다.\n'
         +'수행에 성공할 경우 array로 이루어진 array를 리턴하며, 실패할 경우 에러 e를 throw합니다.';
      function rawQueryForMap(sql,selectionArgs,stringFlag){
         try{
            return cursorToMap(rawQueryForCursor(sql,selectionArgs),stringFlag);
         }catch(e){throw e;}
      }
      rawQueryForMap.toString=()=>'DBManager.rawQueryForArray(string sql)\nDBManager.rawQueryForArray(string sql, string[] selectionArgs, boolean stringFlag)\n'
         +'sql을 실행합니다. select의 수행에 사용하며 insert, update, delete, create, alter등은 execSQL을 이용합니다.\n'
         +'sql 문자열에 ? 가 포함되어 있으면 해당 부분은 selectionArgs의 값으로 대체됩니다.\n'
         +'stringFlag의 값이 true일 경우 column의 type이 String이 아니더라도 String으로 반환됩니다.\n'
         +'수행에 성공할 경우 array로 이루어진 array를 리턴하며, 실패할 경우 에러 e를 throw합니다.';
      ...
      selectForCursor.toString=()=>'DBManager.selectToCursor(string table, string[] columns, string selection, string[] selectionArgs, object options)\n'
         ...
         +'기본 option은 {distinct:false, groupBy:null, having:null, orderBy:null, limit:null}로 되어있으며, 바꾸고자 하는 option이 필요한 경우에만 전달하면 됩니다.\n'
      ...
      function selectForObject(table, columns, selection, selectionArgs, options){
         var stringFlag = (options && options.stringFlag) || false;
         try{
            return cursorToObject(selectForCursor(table, columns, selection, selectionArgs, options),stringFlag);
         }catch(e){throw e;}
      }
      selectForObject.toString=()=>'DBManager.selectForObject(string table, string[] columns, string selection, string[] selectionArgs, object options)\n'
         ...
         +'"selection"이 생략 될 경우 table의 모든 데이터가 select 대상이 됩니다.\n'
         +'"options"은 select문에 필요한 추가 옵션을 주기 위해 사용하는 객체입니다.\n'
         +'기본 option은 {distinct:false, groupBy:null, having:null, orderBy:null, limit:null, stringFlag:false}로 되어있으며, 바꾸고자 하는 option이 필요한 경우에만 전달하면 됩니다.\n'
         +'stringFlag의 값이 true일 경우 column의 type이 String이 아니더라도 String으로 반환됩니다.\n'
      ...
      function selectForArray(table, columns, selection, selectionArgs, options){
         var stringFlag = (options && options.stringFlag) || false;
         try{
            return cursorToArray(selectForCursor(table, columns, selection, selectionArgs, options),stringFlag);
         }catch(e){throw e;}
      }
      ...
      function selectForMap(table, columns, selection, selectionArgs, options){
         var stringFlag = (options && options.stringFlag) || false;
         try{
            return cursorToMap(selectForCursor(table, columns, selection, selectionArgs, options),stringFlag);
         }catch(e){throw e;}
      }
      selectForMap.toString=()=>'DBManager.selectForArray(string table, string[] columns, string selection, string[] selectionArgs, object options)\n'
         +'select를 실행합니다.\n'
         +'"table"은 select의 대상이 되는 table의 이름입니다.\n'
         +'"columns"은 select의 결과로 얻고자 하는 column들의 name으로 이루어진 배열입니다. null일 경우 모든 column이 대상이 됩니다.\n'
         +'"selection"는 select하고자 하는 데이터들의 조건문 입니다. 문자열에 ? 가 포함되어 있으면 해당 부분은 selectionArgs의 값으로 대체됩니다.\n' 
      ...
      createWithID.toString=()=>'DBManager.createWithID(string table, object values)\n'
         ...
         +'수행에 성공할 경우 true를 리턴하며, 실패할 경우 에러 e를 throw합니다.\n'
         +'더 상세한 schema의 table을 생성하기 위해서는 execSQL 사용을 권장합니다.'
      function cursorToObject(cursor,stringFlag){
         ...
         do{
            ...
            for(var i=0;i<columnCount;i++){
               ...
               else if(!stringFlag && (type==FIELD_TYPE_INTEGER || type==FIELD_TYPE_FLOAT))
                  obj[columnNames[i]] = cursor.getDouble(i);
               else
      ...
      function cursorToArray(cursor,stringFlag){
         const count=cursor.getCount();
         const columnCount=cursor.getColumnCount();
         var columnNames=new Array();
         //var types=new Array();
         ...
         do{
            ...
            for(var i=0;i<columnCount;i++){
               ...
               if(type==FIELD_TYPE_NULL)
                  ...
                  sub_arr.push(cursor.getDouble(i));
               else
      ...
      function cursorToMap(cursor,stringFlag){
         const count=cursor.getCount();
         const columnCount=cursor.getColumnCount();
         var columnNames=new Array();
         //var types=new Array();
         var type;
         const map = new Map();
         if(!cursor.moveToNext()) return map;
         for(var i=0; i<columnCount; i++){
            columnNames.push(cursor.getColumnName(i));
         }
         do{
            ...
            if(type==FIELD_TYPE_NULL)
               ...
               key = cursor.getDouble(0);
            else
               key = String(cursor.getString(0));
            for(var i=1;i<columnCount;i++){
               type = cursor.getType(i);
               if(type==FIELD_TYPE_NULL)
                  value[columnNames[i]] = null;
               else if(type==FIELD_TYPE_INTEGER || type==FIELD_TYPE_FLOAT)
                  value[columnNames[i]] = cursor.getDouble(i);
               else
                  value[columnNames[i]] = String(cursor.getString(i));
            }
            map.set(key,value);
         }while(cursor.moveToNext());
         cursor.close();
         Object.defineProperty(map,"columnNames",{value : columnNames.slice(1)});
         return map;
      }
      function arrayToString(arr){
         return   (arr.length==0) ? '검색 결과 없음' : '| ' + arr.columnNames.join(' | ') + ' |\n' +
            arr.map((v)=>{
               return '| ' + v.join(' | ') + ' |';
            }).join('\n')
      }
      function cursorToString(cursor){
         return   arrayToString(cursorToArray(cursor,true));
      }
      function toString(){
