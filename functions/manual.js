func = function (r) {
	if (r.msg.split(" ")[1] == "최근채팅") {
        ...
        r.replier.reply("먹을 음식을 추천해 줍니다. 테마만 고르면 6가지의 먹을거리를 추천해줍니다.");
    } else if (r.msg.split(" ")[1] == "시간") {
        r.replier.reply("[!시간]만 쳤을 경우 대한민국 시간만 나옵니다. [!시간 지역명]으로 검색하면 우리나라와 해당 나라의 시간과 시차를 출력합니다.");
    } else if (r.msg.split(" ")[1] == "명단") {
        r.replier.reply("이 방에서 채팅을 한 번이라도 쳤던 명단을 보여줍니다. [!명단]만 치면 전체 명단이 나오고 [!명단 이름]을 치면 이름이 포함된 명단만 나옵니다.");
    } else if (r.msg.split(" ")[1] == "사진조회") {
        r.replier.reply("[!사진조회]를 통해 최근 사진을 조회 할 수 있습니다. [!사진조회]를 통해 사진을 최근 20개까지 조회 할 수 있습니다. [!사진조회 이름]을 통해 해당하는 사람이 올린 최근 사진을 확인할 수 있습니다. [!사진조회 userid]를 통해 해당하는 사람이 올린 최근 사진을 확인할 수 있습니다.");
    } else if (r.msg.split(" ")[1] == "닉변내역") {
        r.replier.reply("닉네임 내역을 확인합니다. [!닉변내역 userid]를 통해 확인합니다.");
    } else if (r.msg.split(" ")[1] == "아이디조회") {
        r.replier.reply("닉네임을 통해 아이디를 조회합니다. [!아이디조회 닉네임]을 통해 확인합니다.");
    } else if (r.msg.split(" ")[1] == "출입내역") {
        r.replier.reply("이 방의 출입내역을 조회합니다.");
    ...
    } else if (r.msg.split(" ")[1] == "가사") {
        r.replier.reply("[!가사 노래명]과 같이 검색하면 주요 포털 검색결과를 통해 가사를 찾아줍니다. 가사 데이터가 없을 수도 있습니다.");
    } else if (r.msg.split(" ")[1] == "제이플라") {
        r.replier.reply("최신 제이플라 노래를 보여줍니다.");
    } else if (r.msg.split(" ")[1] == "쪽지") {
        r.replier.reply("!쪽지 대상\n쪽지내용\n와 같이 보내면 대상이 채팅을 치는 순간 쪽지가 전달됩니다. [!명단]을 통해 방의 닉네임 목록을 확인할 수 있습니다.");
    ...
    } else if (r.msg.split(" ")[1] == "실검") {
        r.replier.reply("!실검을 치면 네이버 전체나이대 실시간 검색어를 보여줍니다. !실검 10 이런식으로 치면 10대 실시간 검색어를 보여줍니다.");
    	...
    	var str = '';
    	str += '로또 설명서\n'+es+'\n';
    	str += "[!로또]와 같이 치면 로또번호를 랜덤으로 뽑아줍니다.\n[!로또 숫자]의 경우 5장까지는 뽑은 번호를 바로 알려주고 5장 초과 20000장 이하의 경우 뽑은 데이터는 저장되고 별도로 알려주지는 않습니다.\n" 
    	str += "[!당첨]으로 토요일에 로또번호 추첨이 끝나면 결과를 확인할 수 있습니다.\n"
    	str += "[!당첨 닉네임] 과 같이 입력하면 자기가 뽑은 번호만 확인 할 수 있습니다.\n"
    	str += "[!행복회로]는 이번주에 뽑은 번호가 저번주에 뽑았다면 몇 등일지 보여줍니다.\n"
    	str += "[!로또통계]를 통해 지금까지 뽑았던 로또의 당첨내역을 확인할 수 있습니다.\n"
    	str += "[!종합로또통계]는 피치봇이 들어있는 모든방에서 뽑은 로또의 통계입니다.\n"
    	str += "[!로또개수목록]은 이 방에서 이번주에 각자 몇 개를 뽑았는지에 대한 리스트입니다.\n"
    	str += "[!내로또]는 이번주에 내가 뽑은 번호들의 리스트 입니다."
    	r.replier.reply(str);
