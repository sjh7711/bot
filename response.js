...
function response(room, msg, sender, isGroupChat, replier, imageDB) {
	...
	try {
        ...
        if( D.selectForArray('blackjack', 'name', 'room=?', room) == undefined || D.selectForArray('blackjack', 'name', 'room=?', room).map(v=>v[0]).indexOf(sender) == -1 ){
    		D.insert('blackjack', {name : sender  , room : room, point : 10000000, win : 0, lose : 0, push : 0 , ddl : 0, ddw : 0, ddp : 0, blackjack : 0 , even : 0 , evenc : 0, insurc : 0, insur : 0, splitc : 0 , split : 0, sur : 0, allp : 0 });
    	}
...
여러분들은 [!블랙잭] 을 통해 게임을 시작 할 수 있으며 [!블랙잭] 을  외친 사람은 자동으로 참가가 됩니다. 최대 3인까지 가능합니다.\n\
배당율은 기본 1배입니다. 블랙잭인 경우 1.5배입니다. 카드 숫자의 합이 21에 가까운 사람이 이기는 게임입니다. K, Q, J는 10에 해당하며, A는 1 혹은 11 어느 쪽으로도 계산할 수 있습니다.\n\
카드 두 장을 기본적으로 지급받게 되며, 처음 받은 2장 합쳐 21이 나오는 경우 블랙잭이 되며 21이 되지 않았을 경우 원한다면 얼마든지 카드를 계속 뽑을 수 있습니다.\n\
...
[더블다운]은 카드를 무한정 받지 못하고 카드를 단 한 장만 더 받습니다. 배팅액이 두배가 됩니다.\n\
[스플릿]은 같은 숫자인 카드를 받았을 경우 두 장을 나눠서 따로 게임을 진행하는 것입니다. 게임을 따로 진행하는 만큼 배팅액을 추가로 걸게됩니다. A를 제외한 나머지 카드들은 스플릿을 3번 까지 할 수 있습니다. A는 1번만 가능합니다. 스플릿을 하게 되면 기본적으로 2장을 채우기 위해 카드를 하나씩 더 받게됩니다.(미구현)\n\
[서렌더]는 이길 가능성이 낮아서 게임을 포기하는 것 입니다. 배팅액의 절반을 돌려받고 게임을 포기합니다.\n\
...
function blackjack(r){
	...
	if( r.msg == '!블랙잭'){
		if( gameinfo.start == 0 && gameinfo.start1 == 0 &&  gameinfo.start2 ==  0 &&  gameinfo.start3 ==  0 && D.selectForArray('blackjack', 'point', 'name=? and room=?', [r.sender, r.room])[0][0] >= 10000  ){
			...
			Flag.set("gameinfo", r.room , gameinfo);
			r.replier.reply(r.sender+"님("+String(D.selectForArray('blackjack', 'point', 'name=? and room=?', [r.sender, r.room])[0][0]).replace(/(\d{1,3})(?=(\d{3})+$)/g,"$1,")+')이 참가하셨습니다. 현재 1명');
	...
	if (r.msg == '참가' &&  gameinfo.start == 1 && gameinfo.playerlist.indexOf(r.sender) == -1 ){//참가모집중
        if( D.selectForArray('blackjack', 'point', 'name=? and room=?', [r.sender, r.room])[0][0] >= 10000 ){
        	...
        	gameinfo.playerlist.push(r.sender);
            r.replier.reply(r.sender+"님("+String(D.selectForArray('blackjack', 'point', 'name=? and room=?', [r.sender, r.room])[0][0]).replace(/(\d{1,3})(?=(\d{3})+$)/g,"$1,")+")이 참가하셨습니다. 현재 "+gameinfo.playerlist.length+'명');
        } else if (D.selectForArray('blackjack', 'point', 'name=? and room=?', [r.sender, r.room])[0][0] < 10000 ){
	...
	if ( gameinfo.start == 1 && (gameinfo.playerlist.length == 4 || (r.msg == '시작' && gameinfo.playerlist.indexOf(r.sender) > -1 )) ){
		var figure = ['♣','♠','♦','♥'];
	...
	if( gameinfo.start1 == 1 &&  gameinfo.betlist.length < gameinfo.playerlist.length && /^\d+$/.test(r.msg) && gameinfo.playerlist.indexOf(r.sender) > -1 && gameinfo.betlist.indexOf(r.sender) == -1 && gameinfo['player'+num].bet == 0){
		if ( (Number(r.msg)>9999 && Number(r.msg)<500001) || (Number(r.msg)>0 && Number(r.msg)<51)){
			...
			r.replier.reply(gameinfo['player'+num].name+'님이 '+String(gameinfo['player'+num].bet).replace(/(\d{1,3})(?=(\d{3})+$)/g,"$1,")+'원을 배팅.');
