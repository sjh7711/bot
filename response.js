...
var RS = T.register("reactionSpeed",()=>{
	...
	while(1){
		...
		if(this["flag" + r.room][4] == 1 && r.msg == '.'){
			...
			r.replier.reply(r.sender+"님의 반응 속도 : "+ (now1-now-288)/1000 +'초');
...
function func(r) {
    ...
    if (r.msg.split(" ")[1] == "추첨") {
        r.replier.reply("!추첨을 입력하면 참여한 사람 몇 명을 뽑을 건지 입력할 수 있습니다. 입력 후에는 참가를 입력하면 참가가 가능하고, 추첨을 제안한 사람이 !마감을 입력하면 당첨자가 바로 발표됩니다.");
    }
    if (r.msg.split(" ")[1] == "명단") {
