...
function response(room, msg, sender, isGroupChat, replier, imageDB) {
	...
	try {
    	...
    	if ((msg == "!야구" && work == 1) || ( (Flag.get('start', r.room) == 1 || Flag.get('start1', r.room) == 1 ||  Flag.get('start2', r.room) ==  1||  Flag.get('start3', r.room) ==  1) && ( !isNaN(msg) || msg == '참가' || msg == '시작' || msg == '!강제종료' || msg == '!힌트' || msg == '!패스') ) ){
        	baseball(r);
        }
...
function blackjack(r){
	...
	if( gameinfo.start2 == 1 && gameinfo.playerlist.length > 0 ){
		...
		if( r.msg == '더블다운'){
			r.replier.reply(r.sender+'님의 더블다운.');
