        ...
        if(  room =='test' || room =='시립대 봇제작방' || room =='BASEBALL' || room =='오버워치' || room =='공익' || room =='시립대 전전컴 톡방' ){
        	...
        	if(msg == '!정보'){
        		if(D.selectForArray('baseball',null,'name=? and room=?',[sender, room])!=undefined){
        			...
        			+'\n'+sender+'님의 야구 승률 : '+ Number(Math.floor((Number(D.selectForArray('baseball', 'win','name=? and room=?',[sender, room])) / (Number(D.selectForArray('baseball', 'lose','name=? and room=?',[sender, room]))+Number(D.selectForArray('baseball', 'win','name=? and room=?',[sender, room]))))*1000)/10)+'%';
        			return;
        		}else {
