...
[!강제종료]를 통해 게임을 강제로 종료할 수 있습니다. 혼자 플레이 중인 경우 아무나 종료 가능하고 2인 이상일 경우 현재 참가중인 플레이어 중에서만 강제종료가 가능합니다.')
    } else if (r.msg.split(" ")[1] == "뽑기"){
    	r.replier.reply("기본값은 1~100이고 !뽑기 200 처럼하면 1~200까지, !뽑기 2 200 처럼하면 2부터 200까지 랜덤한 숫자를 뽑습니다.");
    }
}
...
function baseball(r){
	if( (Flag.get('start', r.room) == 1 || Flag.get('start1', r.room) == 1 ||  Flag.get('start2', r.room) ==  1) && r.msg == '!강제종료' && ( (Flag.get('baseballtime', r.room ) + 6000*8) < new Date().getTime() ) && Flag.get('baseball', r.room).length > 1 ){
		...
		return;
	} else if ((Flag.get('start', r.room) == 1 || Flag.get('start1', r.room) == 1 ||  Flag.get('start2', r.room) ==  1) && r.msg == '!강제종료' && ( (Flag.get('baseballtime', r.room ) + 6000*8) > new Date().getTime() ) && Flag.get('baseball', r.room).length > 1 ){
		r.replier.reply( (Math.floor( (Flag.get('baseballtime', r.room ) + 6000*8 - new Date().getTime()) * 10000 )/10 ) + '초 뒤에 강제종료가 가능합니다.');
	}
